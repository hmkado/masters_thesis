...
imwrite(img,sprintf("trial_%d/base.png",trial_no));
...
for n = testbots
    ...
    [~,station] = min(sum(abs(mh - [c h]),2));
    agv(n).assign = station;
    if (c == 1 || c == ncolrow(2)) && (h == 1 || h == ncolrow(1))
        stat(n) = 1;
    else
        stat(n) = 0;
    end
...
istep = istep + 1;
imwrite(img,sprintf("trial_%d/si_%d.png",trial_no,istep));
%% Begin return
for n = testbots
    if stat(n) == 0
        stat(n) = 1;
        agv(n).dest = mh(agv(n).assign,:);
    else
        stat(n) = 0;
    end
end
